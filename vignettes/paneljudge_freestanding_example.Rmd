...
vignette: >
  %\VignetteIndexEntry{paneljudge_freestanding_example}
...
```{r setup}
devtools::load_all() # Remove this line once complete
...
av_marker_length <- mean(markers$length) # Average marker length
...
for(chr in unique(markers$chrom)){
  ...
  # Add applicon positions (barely visible on whole genome scale)
  ind <- markers$chrom == chr
  segments(x0 = markers$Start[ind], x1 = markers$Stop[ind],
           y0 = chr, y1 = chr, col = 'white')
}
```

The markers in the example panel are microhaplotypes: short, highly diverse regions of the genome whose length can be spanned by a single amplicon. The white marks in the above plot mark the region spanned by each (marker/amplicon? - check). They are barely visible due to their small size (average `r round(av_marker_length)` base pairs; standard deviation: `r round(sd_marker_length)` base pairs). Unfortunately, some markers are very close together (minimum distance between marker mid-points `r min(markers$dt)` base pairs). However, none overlap: 
...
hist(markers$dt, col = 'gray', main = ' ', las = 1, 
...
Using the example panel and marker allele frequencies, for various data-generating values of relatedness, $r$, we first simulate some data on a pair of haploid genotypes; second estimate $r$ (and a switch rate parameter, $k$, whose data-generating value is fixed in this vignette); and third compute 95\% confidence intervals (CIs) around the estimates $\hat{r}$ and $\hat{k}$:
...
distances <- markers$dt # distances between marker mid-points

mle_CIs <- lapply(rs, function(r) {
  sapply(1:n, function(i) {

    # First simulate genotype pair
    Ys <- simulate_data(fs, gendist = distances, k, r)

    # Second, estimate r and k
    krhat <- estimate_r_and_k(frequencies = fs, distances, Ys)

    # Third, compute confidence intervals (CIs)
    CIs <- compute_r_and_k_CIs(frequencies = fs, distances, khat = krhat['khat'], rhat = krhat['rhat'])
...
The variation in $\hat{r}$ around the data-generating $r$ (black horizontal line) is partly due to limited panel informativeness and partly due to the finite length of the genome, i.e. Mendelian sampling @hill2011. Whole genome simulation is required to partition these two sources of variation (see note below).
